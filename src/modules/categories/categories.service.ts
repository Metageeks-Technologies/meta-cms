...
export class CategoriesService {
  ...
  async findById(websiteKey: string, _id: string) {
    ...
    const category = await this.Category.findOne({ _id: _id, websiteKey: websiteKey }).lean().exec();
    if (!category) {
  ...
  async updateById(websiteKey: string, _id: string, updatedCategoryData: UpdateCategoryDto) {
    try {
      ...
      const query = await this.Category.updateOne({ _id: _id, websiteKey: websiteKey }, { $set: updatedCategoryData }).exec();
  ...
  async deleteById(websiteKey: string, _id: string) {
    ...
    const query = await this.Category.deleteOne({ _id: _id, websiteKey: websiteKey }).exec();
