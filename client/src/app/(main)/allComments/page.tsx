...
const CommentsPage: React.FC = () => {
    ...
    // Using global styledScrollable class instead of defining custom scrollbar

    return (
        <div className="w-full p-3 sm:p-4 md:p-6 mx-auto bg-black text-white mb-9 min-h-screen flex flex-col">
            {/* Responsive filter buttons - will scroll horizontally on small screens */}
            <div className="mb-6 overflow-x-auto pb-2 styledScrollable">
                <div className="flex space-x-2 md:space-x-4 min-w-max">
                    {['awaiting approval', 'published', 'rejected', 'deleted'].map(status => (
                        <div
                            ...
                            className={`bg-gray-900 px-2 py-1 sm:px-3 sm:py-2 rounded-lg border border-gray-800 flex items-center gap-1 sm:gap-2 cursor-pointer whitespace-nowrap ${
                                filter === status ? 'text-blue-800 border-blue-800 bg-blue-500' : 'text-white'
                            }`}
                        >
                            {filter === status && (
            ...
            <div className="flex-1 overflow-hidden flex flex-col">
                {comments.length === 0 ? (
                    <div className="flex flex-1 items-center justify-center py-8 sm:py-12 md:py-16 bg-gray-900/50 rounded-lg border border-gray-800">
                        <p className="text-gray-500 text-base sm:text-lg md:text-xl text-center italic">No comments at this time.</p>
                    </div>
                ) : (
                    <div className="flex-1 overflow-y-auto pr-1 styledScrollable">
                        <ul className="space-y-3 md:space-y-4 pb-4">
                            {comments.map(comment => (
                                <li key={comment._id} className="border border-gray-800 rounded-lg shadow-lg overflow-hidden bg-gray-900/80 hover:bg-gray-900 transition-colors duration-200">
                                    <div className="bg-gray-800/60 px-3 sm:px-4 py-2 sm:py-3 border-b border-gray-700 flex flex-col sm:flex-row sm:justify-between sm:items-center">
                                        <h2 className="text-xs sm:text-sm md:text-base font-semibold text-blue-300 truncate">
                                            {comment.postDetails.title}
                                        </h2>
                                        <p className="text-xs text-gray-400 mt-1 sm:mt-0">
                                            <span className="bg-gray-900 px-1.5 py-0.5 rounded-full">
                                                {comment.userDetails?.name || "Anonymous"}
                                            </span> • <em className="text-xs">{formatDate(comment.createdAt)}</em>
                                        </p>
                                    </div>
                                    <div className="p-3 sm:p-4">
                                        ...
                                        {editingCommentId === comment._id ? (
                                            <>
                                                <textarea
                                                    className="w-full p-2 sm:p-3 text-xs sm:text-sm bg-gray-800 text-gray-200 border border-gray-700 rounded-md focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none resize-none"
                                                ...
                                                <div className="mt-2 sm:mt-3 flex space-x-2">
                                                    <button
                                                        className="text-xs bg-blue-600 text-white px-3 py-1 sm:px-4 sm:py-1.5 rounded-md hover:bg-blue-500 transition-colors shadow-sm flex items-center gap-1.5"
                                                    ...
                                                    <button
                                                        className="text-xs bg-gray-700 text-gray-200 px-3 py-1 sm:px-4 sm:py-1.5 rounded-md hover:bg-gray-600 transition-colors shadow-sm"
                                                        onClick={() => {
                                                            setEditingCommentId(null); 
                                                            setEditingMessage(''); 
                                                        }}
                                            ...
                                            <div className="text-xs sm:text-sm text-gray-200 bg-gray-800/70 p-2 sm:p-3 rounded-md mb-3 sm:mb-4 border-l-4 border-blue-500">
                                                {comment.message}
                                            </div>
                                        )}
                                        
                                        <div className="mt-3 sm:mt-4 border-t border-gray-800 pt-3 sm:pt-4 flex flex-wrap gap-2">
                                            {(comment.status === 'awaiting approval' && !comment.isDeleted) && (
                                                <>
                                                    <button
                                                        className="text-xs bg-green-600 text-white px-2 py-1 sm:px-3 sm:py-1.5 rounded-md hover:bg-green-500 transition-all duration-200 shadow-sm flex items-center gap-1"
                                                    ...
                                                    <button
                                                        className="text-xs bg-red-600 text-white px-2 py-1 sm:px-3 sm:py-1.5 rounded-md hover:bg-red-500 transition-all duration-200 shadow-sm flex items-center gap-1"
                                                    ...
                                                    <button
                                                        className="text-xs bg-gray-700 text-white px-2 py-1 sm:px-3 sm:py-1.5 rounded-md hover:bg-gray-600 transition-all duration-200 shadow-sm flex items-center gap-1"
                                            ...
                                            {comment.status === 'rejected' && (
                                                <button 
                                                    ...
                                                    className="text-xs bg-blue-600 text-white px-2 py-1 sm:px-3 sm:py-1.5 rounded-md hover:bg-blue-500 transition-all duration-200 shadow-sm flex items-center gap-1"
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3" viewBox="0 0 20 20" fill="currentColor">
                ...
                {hasMore && (filter === 'published' || filter === 'rejected' || filter === 'deleted') && (
                    <div className="py-4 flex justify-center">
                        <button 
                            ...
                            className="text-xs sm:text-sm bg-blue-600 text-white px-4 py-1.5 sm:px-5 sm:py-2 rounded-md hover:bg-blue-500 transition-all duration-200 shadow-md flex items-center gap-2"
                        >
